...
echo "== Ensuring prerequisites for LP02/M03 (env, ACR, image, container app, Service Bus) =="
...
if az containerapp show --name "$ACA_APP" --resource-group "$RESOURCE_GROUP" --output none 2>/dev/null; then
  ...
  echo "Container app '$ACA_APP' not found - creating..."
...
if az servicebus namespace show --name "$SERVICEBUS_NAMESPACE" --resource-group "$RESOURCE_GROUP" --output none 2>/dev/null; then
  echo "Service Bus namespace '$SERVICEBUS_NAMESPACE' already exists."
else
  az servicebus namespace create --name "$SERVICEBUS_NAMESPACE" --resource-group "$RESOURCE_GROUP" \
    --location "$LOCATION" --sku Basic --output table
fi

if az servicebus queue show --namespace-name "$SERVICEBUS_NAMESPACE" --resource-group "$RESOURCE_GROUP" \
     --name "$SERVICEBUS_QUEUE" --output none 2>/dev/null; then
  echo "Service Bus queue '$SERVICEBUS_QUEUE' already exists."
else
  az servicebus queue create --namespace-name "$SERVICEBUS_NAMESPACE" --resource-group "$RESOURCE_GROUP" \
    --name "$SERVICEBUS_QUEUE" --output table
fi
